...
teeth = 41;			// Number of teeth, standard Mendel T5 belt = 8, gives Outside Diameter of 11.88mm
...
m3_nut_flats = 5.5;	// normal M3 hex nut exact width = 5.5
m3_nut_depth = 2.7;	// normal M3 hex nut exact depth = 2.4, nyloc = 4

retainer = 1;		// Belt retainer above teeth, 0 = No, 1 = Yes
retainer_ht = .8;	// height of retainer flange over pulley, standard = 1.5
idler = 1;			// Belt retainer below teeth, 0 = No, 1 = Yes
idler_ht = .8;		// height of idler flange over pulley, standard = 1.5
...
if ( profile == 14 ) { pulley ( "GT2 5mm" , GT2_5mm_pulley_dia , 1.969 , 3.952 ); }
        ...
        translate([8.75,0,0]) {
            cylinder(h=(pulley_t_ht+idler_ht+retainer_ht), r=2.5/2,$fn=50);
        }
        translate([-8.75,0,0]) {
            cylinder(h=(pulley_t_ht+idler_ht+retainer_ht), r=2.5/2,$fn=50);
        }
        translate([8.75,0,0]) {
            cylinder(h=2.75, r=4.5/2,$fn=50);
        }
        translate([-8.75,0,0]) {
            cylinder(h=2.75, r=4.5/2,$fn=50);
        }
        ...
        translate([0,0,3.6]) {
            difference() {
             cylinder(h=(pulley_t_ht+idler_ht+retainer_ht)-3.6, r=motor_shaft/2,$fn=50);
             cylinder(h=(pulley_t_ht+idler_ht+retainer_ht), r=1,$fn=50);
            }
        }
...
////  {{
//     translate([0,0,(pulley_t_ht+idler_ht+retainer_ht)/2]) 
//      {cylinder(h=(pulley_t_ht+idler_ht+retainer_ht)-4*2, r=motor_shaft/2,center=true,,$fn=50);}}
//    {
//        translate([0,0,(pulley_t_ht+idler_ht+retainer_ht)/2]) {
//            cylinder(h=(pulley_t_ht+idler_ht+retainer_ht), r=3.2/2,center=true,$fn=50);}}
// }
//
//}
// Functions
